...
export default function App() {
  ...
  // Onboarding States (Simplificado)
  ...
  useEffect(() => {
    ...
    // Check if user has the new location structure.
    if (savedUser && savedUser.location) {
      ...
      setScreen(AppScreen.FEED); 
        ...
        setScreen(AppScreen.PROFILE); // Força tela de login se não tiver usuário
    }
  ...
  useEffect(() => {
      // Se não tiver user, ainda carregamos pedidos
  ...
  const handleSimpleLogin = async () => {
      if (!onboardingName || !onboardingCity || !onboardingHood || !onboardingState) {
          triggerToast('Por favor, preencha todos os campos.');
          return;
      }
      ...
      const location = {
          ...
          city: onboardingCity,
          state: onboardingState.toUpperCase(),
          country: 'Brasil'
      };

      const newUser = await dbService.registerSimpleUser(onboardingName, location);
      
      setUser(newUser);
      setFilterScope('CITY');
      setScreen(AppScreen.FEED);
      setIsAuthLoading(false);
      triggerToast('Bem-vindo à Liga!');
  ...
  const handleOfferHelp = async (requestId: string) => {
    if (!user) {
        setScreen(AppScreen.PROFILE); // Redirect to login
        triggerToast("Crie seu perfil para oferecer ajuda!");
        return;
    }
    ...
    <div className="pb-10 pt-10 px-6 min-h-screen flex flex-col items-center justify-between animate-in fade-in duration-500 bg-white">
        ...
        <div className="w-full max-w-sm flex flex-col items-center justify-center flex-1">
            ...
            <div className="w-full max-w-sm space-y-3">
                ...
                <div className="relative group">
                    ...
                    <input 
                        ...
                        placeholder="Seu Nome (Ex: Ana Silva)"
            ...
            <div className="mt-8 w-full max-w-sm">
                ...
                >
                    {isAuthLoading ? <Loader2 className="animate-spin" /> : 'Entrar na Liga'}
                </button>
            </div>
        </div>
